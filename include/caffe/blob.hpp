 ...
 *        interact.
 *        BLOB是SyncedMemory的包裹器（wrapper 参考这个: https://zhidao.baidu.com/question/307738931.html）
...
class Blob {
 public:
  ...
  explicit Blob(const int num, const int channels, const int height,
      const int width);
  explicit Blob(const vector<int>& shape); //用这个
  ...
  inline int count(int start_axis, int end_axis) const {
    CHECK_LE(start_axis, end_axis); //保证输入的有效性
   ...
   * @param start_axis The first axis to include in the slice.
   */
  inline int count(int start_axis) const { //给定的维度到最后的维度之间包含的数据数目
  ...
  //计算一维线性偏移量,对于批量图像数据来说， blob 常规的维数为图像数量 N *通道数 K *图像高度 H *图像宽度 W。 
  //Blob **按行为主（row-major）** 进行存储，所以一个 4 维 blob 中，
  //坐标为(n, k, h, w)的值的物理位置为（（n * K + k) * H + h) * W + w，这也使得最后面/最右边的维度更新最快。
  ...
  // 计算一维线性偏移量,但参数在 vector<int> 里，nchw
  ...
  // 同步内存 shared_ptr（不明白share_ptr的可以自行百度，引用计数管理机制）
  //https://blog.csdn.net/u013696062/article/details/39665247
  ...
  const Dtype* gpu_acmean() const;  // added term
  ...
  Dtype* mutable_gpu_acmean();   // added term
  // 计算\[Y=alpha * X +beta*Y \]  
  void Update();
