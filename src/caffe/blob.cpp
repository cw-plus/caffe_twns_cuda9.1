...
extern int TERNARY_DELTA; // 让编译器在本文件另外一个地方或者其他文件里定义了TERNARY_DELTA 在 blob.cpp 里调用

namespace caffe {

// new added start
/**************三值化开始*********************************************************************************************/
//来源:https://github.com/fengfu-chris/caffe-twns/blob/master/src/caffe/blob.cpp
...
vector<int> Blob<Dtype>::ternary_stats(){
	if(ternary_stats_ == NULL){
		ternary_stats_ = new vector<int>(3,0); // 是指 3 个状态吧 -1,0，+1吧
	}
	CHECK_EQ((*ternary_stats_).size(), 3);
	for(int i=0; i<3; i++){
		(*ternary_stats_)[i] = 0;
	}
	float eps = 1e-6; //浮点数 关于==0 的问题要注意不能直接等于
	const Dtype* X = this->cpu_binary(); //？？
	for(int i=0; i<this->count(); i++){
		Dtype x = X[i];
		int id = 2 * (x>eps) + 0 * (x<-eps) + 1*(x>=-eps && x<eps);
    // 正数 0 负数分别输出 2,1,0？？？           
		(*ternary_stats_)[id] += 1; //或者是用来统计正负数和0的??
	}
...
template <typename Dtype>
void Blob<Dtype>::clip_data(){ //调用库函数
    caffe_gpu_clip(count_, mutable_gpu_data()); //限制在 ±1 间
/*
  if (!data_) { return; }
  switch (data_->head()) {
  ...
  case SyncedMemory::HEAD_AT_CPU:
{
  /*  // 我的理解: 把cpu_data() 三值化后放到 mutable_cpu_binary() 里
    ...
    // 分步实现:α=∑|Wi|  (i∈IΔ) 这里巧妙地使用cpu_binary() 这里是符号位，点乘后正好求了绝对值，666
...
void Blob<Dtype>::Reshape(const int num, const int channels, const int height,
  ...
  shape[2] = height;
  shape[3] = width;
  Reshape(shape); // reshape 的具体实现 过时的方法最终是调用的新的reshape方法
}
...
template <typename Dtype>  //这个方法也是不用了，实现时调用新的方法
...
const Dtype* Blob<Dtype>::cpu_data() const {
  CHECK(data_);
  return (const Dtype*)data_->cpu_data(); // 这个是不是调用了 
  // ./include/caffe/syncedmem.hpp 里的 const void* cpu_data()？？？
}
...
template <typename Dtype> // 可以用来改变的数据
...
template <typename Dtype> // 将其他blob的数据复制到当前的blob中去
...
// Blob<int> or Blob<unsigned int>. 意思是模板类型不能是uint 和 int
template <> void Blob<unsigned int>::Update() { NOT_IMPLEMENTED; }
template <> void Blob<int>::Update() { NOT_IMPLEMENTED; }

template <typename Dtype> // Update是计算data=-1 * diff + data
void Blob<Dtype>::Update() {
  ...
  case SyncedMemory::HEAD_AT_CPU:
    ...
    caffe_axpy<Dtype>(count_, Dtype(-1),
        ...
        static_cast<Dtype*>(data_->mutable_cpu_data()));//存储时用mutable_cpu_data，防止其他线程访问
    //实现的表达式：data_->mutable_cpu_data() = -1*diff_->cpu_data() + data_->mutable_cpu_data()
    break;
  case SyncedMemory::HEAD_AT_GPU:
  case SyncedMemory::SYNCED:
#ifndef CPU_ONLY
...
//其实是限制 unsigned int，int 的 asm_data() 的使用？
...
template <typename Dtype> // 计算 data 的 L1 范数
...
template <typename Dtype> // 将 data 乘以一个因子scale_factor
...
template <typename Dtype> // 将 diff 乘以一个因子 scale_factor
...
template <typename Dtype> // 判断两个 blob 的shape 是否一样
